...
class ReactConfig:
    ...
    ## threshold of 1 means that there are no hallucinations and no instructions violated
    ## groundedness score should be above or at this threshold to be considered acceptable
    HALLUCINATION_THRESHOLD: float = 1
    ## instruction adherence score should be above or at this threshold to be considered acceptable
    IA_THRESHOLD: float = 1
...
class Coordinator:
    ...
    def end_reprom(self, aimon_payload, result, iteration_num):
        ...
        if iteration_num >= self.react_configuration.max_attempts:
            ...
            return True
        ## continues reprompting if there is hallucination
        failed_instructions = self.reprompter.get_failed_instructions(result, aimon_payload)
        if failed_instructions:
            logger.info("Groundedness or instruction adherence violations detected. Continuing reprompting.")
            return False
        
        ## logs reason for stopping reprompting
        logger.info("No Hallucinations and all instructions are followed")
        return True
    ...
    def get_residual_error_score(self, result):
        ...
        a score of 0 means that the response is perfect, i.e. no hallucinations and all instructions are followed
        """
        return round(2 - result.detect_response.groundedness["score"] - result.detect_response.instruction_adherence["score"],2)
    
    # generates a vector of errors and scores for telemetry
    ## can be optimized to use reprompter get_failed_instructions
    def gen_error_vector(self, result):
        # Store groundedness and instruction adherence scores
        scores = {
        ...
        # Collect all instruction failures from instruction adherence
        errors = [
            {
        ...
        # Collect all failures from groundedness
        groundedness_errors = [
            {
                "type": "instruction_failure",
                "source": "groundedness",
        ...
        # Combine both
        errors.extend(groundedness_errors)
    ...
    def coordinate(self, llm_num, context, user_query, user_instructions):
        ...
        ## evaluate response on hallucination/groundedness, instruction adherence, and toxicity
